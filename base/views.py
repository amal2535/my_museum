...
def avgRate(ch):
    room_messages = Comment.objects.filter(Q(room__name__icontains=ch))        
    a=0
    b=0
    c=0
    d=0
...
def update_average_rating(request):
    ...
    for musee in musees:
        ...
        Museum.objects.filter(name__icontains=musee.name).update(avg_rating = avgRate(musee.name))

        print(musee.name,' : ', avgRate(musee.name))
